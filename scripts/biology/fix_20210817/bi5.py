...
cluster_run = False
####
input_nc_dir = r'D:\MPA_connectivity_BACKUP\scripts\sim1101\outputs\nc' # where nc files were output
input_npy_dir = r'D:\MPA_connectivity_BACKUP\scripts\sim1101\outputs\npy' # where npy files were output
####
shp_og = r'D:\MPA_connectivity_BACKUP\inputs\mpas\mpa_.shp' # full release_polys dataset
shp_og_buff = r'D:\MPA_connectivity_BACKUP\inputs\mpas\mpa_buff.shp'  # buffered for checking settlement
####
crs_input_shp = {'init' :'epsg:3005'} # BC Albers
...
if cluster_run:  # nc_group gets added for cluster runs
    ...
    nc_index = None
    ncs = ncs[nc_group-1:nc_group] # to get just the one indicated by nc_group
