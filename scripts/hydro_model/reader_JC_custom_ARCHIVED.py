...
from dateutil import parser  # !!!JC EDIT!!!
...
class Reader(BaseReader, UnstructuredReader):
    ...
    def __init__(self, filename=None, name=None, proj4=None):
        ...
        # assert self.dataset.CoordinateSystem == "Cartesian", "Only cartesian coordinate systems supported" #!!! JC EDITS !!!

        self.x = self.dataset['x'][:]
        self.y = self.dataset['y'][:]
        # self.xc = self.dataset['xc'][:]   #!!! JC EDITS !!!
        ...
        #!!! JC EDITS !!!
        # this is a bit hokey, but for the sake of time:
        units = self.dataset['time'].units
        interval = units.split()[0]
        if not interval=='hours': # hours gets hardcoded below
            raise ValueError('JC: Interval is not in hours. Revisit reader script')
        time_or = units.split(' ', 2)[2]
        ref_time = parser.parse(time_or)
        ...
        # ])
        self.times = np.array([
            ref_time + timedelta(hours=d.item())
            for d in self.dataset['time'][:]
        ]) # !!! JC EDITS !!! NEP and SSC are in hours since ref date
        ...
        # self.faces_idx = self._build_ckdtree_(self.xc, self.yc) # !!! JC EDIT !!!
